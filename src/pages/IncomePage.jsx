...
export default function IncomePage() {
  ...
  const monthSummary = MONTHS.map((label, i) => {
    const total = rows.filter(r => r.month === i+1).reduce((s,r) => s+Number(r.amount), 0)
    return { label, total, month: i+1 }
  })
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        const { error: e } = await supabase.from('income').update(payload).eq('id', editing)
        if (e) throw e
      } else {
        const { error: e } = await supabase.from('income').insert(payload)
        if (e) throw e
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-white">Income</h1>
          <p className="text-muted text-sm mt-0.5">Annual total: <span className="text-emerald-400 font-mono">{fmt(annualTotal)}</span></p>
        </div>
      ...
      <div className="grid grid-cols-6 md:grid-cols-12 gap-2">
        {monthSummary.map(({ label, total }) => (
          <div key={label} className="rounded-xl border border-border p-2 text-center"
            style={{ background: total > 0 ? 'rgba(63,185,80,0.06)' : '#161B22',
                     borderColor: total > 0 ? 'rgba(63,185,80,0.25)' : '#21262D' }}>
            <p className="text-xs font-mono text-muted">{label}</p>
            <p className="text-xs font-bold mt-1" style={{ color: total > 0 ? '#3FB950' : '#21262D' }}>
      ...
      <div className="rounded-2xl border border-border overflow-hidden" style={{ background:'#161B22' }}>
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-muted text-sm">No income entries for {filterYear}</p>
            <button onClick={openAdd} className="mt-3 text-accent text-sm hover:underline">Add your first entry</button>
          </div>
        ) : (
          <table>
            <thead>
        ...
        <Modal title={editing ? 'Edit Income' : 'Add Income'} onClose={() => setModal(false)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-muted mb-1.5 font-mono uppercase tracking-wider">Year</label>
                <select value={form.year} onChange={e => f('year', Number(e.target.value))}>
                  {[2024,2025,2026,2027,2028,2029,2030].map(y => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs text-muted mb-1.5 font-mono uppercase tracking-wider">Month</label>
            ...
            <div>
              <label className="block text-xs text-muted mb-1.5 font-mono uppercase tracking-wider">Source</label>
